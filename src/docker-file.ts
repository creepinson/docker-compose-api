...
class DockerFile {
	...
	constructor(
		private fp: string = 'tests/docker/test.dockerfile/Dockerfile'
	) {}
	setPath(p: string) {
		this.fp = p;
		return this;
	...
	setOnBuild(onb: IDockerFile['ONBUILD']) {
		...
		const fd: string[] = (keys as string[]).map((inst: string) => {
			const data = (onb as unknown)[inst];
			if (isArrayl(data)) {
	...
	pushValue(key: string, v: unknown) {
		let target = (this.df as unknown)[key];
		if (target && !isArrayl(target)) {
			target = [target].concat(v) as string[];
		} else if (target && isArrayl(target)) {
			if (isArrayl(v)) {
				(v as string[]).forEach((i) =>
					(this.df.RUN as string[]).push(i)
				);
			} else {
				(target as string[]).push(v as string);
			}
		}
		(this.df as unknown)[key] = target || v;
	...
	set(opts: IDockerFile) {
		...
		props.forEach((p: string) => {
			const key = p.includes('_') ? p.split('_').slice(0, 1).join('') : p;
			(this as unknown)[key]((opts as unknown)[p]);
		});
		return this;
	}
